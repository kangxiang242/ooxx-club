    ...
    public function form()
    {
        $this->tab('人物參數',function (){
            ...
            $this->array('rules','分類匹配規則', function (NestedForm $table) {
                $table->select('field','字段')->options(Category::where('parent_id','>',0)->pluck('name','id'));
                $table->select('operator','運算符')->options(['>='=>'>=','<='=>'<=','='=>'=','random'=>'隨機以下各項']);
            ...
            $this->array('birthplace_rules','茶籍分類互斥', function (NestedForm $table) {
                $table->select('field','字段')->options(Birthplace::where('status',1)->pluck('name','id'));
                $table->select('operator','運算符')->options(['except'=>'排除以下各項']);
