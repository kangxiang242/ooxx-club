    ...
    <a href="{{ url('product/'.$goods->id) }}">
        <div class="cover">
            ...
            <div class="master {{ $current_count%7==0?"have-video":"" }}" >
                <img class="goods-img blur" fetchpriority="low" src="{{ imageToBase64('/uploads/'.$goods->thumbnail('small','cover')) }}" data-lazyload data-src="{{ asset_upload($goods->thumbnail('450','cover')) }}" data-cover-id="{{ $goods->id }}" alt="{{ $goods->name }}">
                @if($current_count%7==0)
                    <video preload="metadata" class="g-video" style="object-fit:cover;" src="{{ asset_upload($goods->video) }}" autoplay loop="" muted="" width="100%" playsinline="" >
                    </video>
                @endif
            </div>
        </div>
