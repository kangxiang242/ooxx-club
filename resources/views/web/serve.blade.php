...
<div class="infobody">
    <div class="pic">
        ...
        <img id="cover" src="{{ asset_upload($product->thumbnail('small','cover')) }}" data-src="{{ asset_upload($product->thumbnail('small','cover')) }}" alt="{{ $product->name }}" style="width: 100%;    position: relative;z-index: 2;">
        ...
        <ul class="spec-list" style="z-index: 2">
            @foreach(explode(',',$product->picture) as $key=>$item)
                <li class="list-pic {{ $key==0?"chosen":"" }}">
                    <img src="{{ asset_upload($product->thumbnail('small',$item)) }}" data-src="{{ asset_upload($item) }}" alt="{{ $product->name }}">
                </li>
            @endforeach
                <li class="list-vedio">
    ...
    <div class="info" style="z-index:3">
        ...
        <div class="reviewbox">
            ...
            <ul class="reviewlist">
                ...
                <li class="reviewitem">
                    <img src="" data-src="{{ asset_upload($item) }}" alt="客評截圖">
                </li>
                @endforeach
            </ul>
        </div>
